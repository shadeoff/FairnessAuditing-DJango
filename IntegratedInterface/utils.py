...
class UnfairMetric():
    ...
    # def is_unfair(self, x1, x2, y1, y2):
    #     dy_value = self.dy(y1, y2)
    #     dx_value = self.dx(x1, x2)
    #
    #     if dy_value.dim() > 0:
    #         dy_value = dy_value.squeeze()
    #     if dx_value.dim() > 0:
    #         dx_value = dx_value.squeeze()
    #     dy_value = dy_value.item()
    #     dx_value = dx_value.item()
    #     print(f"type of epsilon is {type(self.epsilon)} and value is {self.epsilon}")
    #     return dy_value > dx_value * self.epsilon
