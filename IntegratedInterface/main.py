...
from IntegratedInterface.utils import get_data, UnfairMetric, decide_label_by_majority_voting, add_data_to_dataset
...
class AuditingFramework:
    ...
    def _dict2tensor(self, data_dict):
        ...
        # print(data_df)
        data_feature = torch.tensor(data_df.values.astype(np.float32))
        # print(data_feature)
    ...
    def set_individual_fairness_metric(self, dx, eps):
        ...
        if dx == 'LR':
            ...
            distance_x.fit(self.dataset.get_all_data(), data_gen=self.data_gen,
                           sensitive_idxs=self.dataset.sensitive_idxs)
        else:
            distance_x = ProtectedSEDistances()
            distance_x.fit(num_dims=self.dataset.dim_feature(), data_gen=self.data_gen,
                           sensitive_idx=self.dataset.sensitive_idxs)
    ...
    def optimize(self, model, unfair_pair):
        ...
        addition_data_label = (torch.ones(addition_data.shape[0]) * decide_label_by_majority_voting(unfair_pair, model,
                                                                                                    self.data_gen,
                                                                                                    1000)).detach()
